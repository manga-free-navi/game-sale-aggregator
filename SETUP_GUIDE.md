# 🎮 ゲームナビ 運営・管理用セットアップガイド

本プロジェクト（無料＆割引ゲームセールナビ）を本番環境でリリース・公開し、自動運転を開始するまでに必要な管理者向け設定手順書です。

---

## 🛠️ 1. GitHub Actions (自動運転) の本番設定

GitHub Pages や外部サーバーにサイトを毎日自動デプロイし、最新の無料ゲーム配布情報やストア値引き情報を更新し続けるには、GitHub リポジトリ上で環境変数（Secrets / Variables）を登録する必要があります。

### 設定方法
1. GitHub上のゲームナビ用リポジトリページを開きます。
...
| 区分 | 変数名 | 種別 | 説明 |
| :--- | :--- | :---: | :--- |
| **データ自動収集** | `NEXT_PUBLIC_RAKUTEN_AFFILIATE_ID` | **Secret** | 楽天アフィリエイトID（任意）。ゲーム機の周辺機器や関連商品のアフィリエイトリンク生成に使用されます。 |
...
| | `NEXT_PUBLIC_ANIME_SITE_URL` | **Variable** | 姉妹サイト「アニフリー（アニメ）」の公開URL。 |
| | `NEXT_PUBLIC_GAME_SITE_URL` | **Variable** | 本サイト（ゲームナビ）の公開URL。 |
...
*   **プライバシーポリシー・免責事項の設置済み**: フッターのリンクから閲覧できる簡易プライバシーポリシーセクション ([MainApp.tsx](file:///C:/Users/MASAYUKI/.gemini/antigravity/scratch/game-sale-aggregator/src/components/MainApp.tsx)) に、アドセンスやCookieに関する免責表記があらかじめ設置されています。
...
*   **favicon**: [public/favicon.ico](file:///C:/Users/MASAYUKI/.gemini/antigravity/scratch/game-sale-aggregator/public/favicon.ico) (ブラウザのタブ用アイコン)
*   **PWAアイコン**: [public/icon.svg](file:///C:/Users/MASAYUKI/.gemini/antigravity/scratch/game-sale-aggregator/public/icon.svg) (ホーム画面追加時用のアイコン)
